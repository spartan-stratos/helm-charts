...
| hooks[].deletePolicy                                                                                                                                              | Optional Helm hook delete policy, for example `before-hook-creation` for Argo CD reruns                                                           | null         |
...
| keda.enabled                                                                                                                                                      | Enables KEDA autoscaling                                                                                                                           | false        |
| keda.fallback                                                                                                                                                     | Replica count to hold while the scaler cannot fetch metrics. Needs a trigger that is not cpu or memory                                             | {}           |
| keda.fallback.failureThreshold                                                                                                                                    | Consecutive metric-fetch failures before the fallback engages                                                                                      | null         |
| keda.fallback.replicas                                                                                                                                            | Replica count to hold while the scaler is failing                                                                                                  | null         |
| keda.fallback.behavior                                                                                                                                            | static, currentReplicas, currentReplicasIfHigher or currentReplicasIfLower. Needs KEDA 2.17.0+                                                     | null         |
| keda.minReplicas                                                                                                                                                  | Minimum amount of Replicas                                                                                                                         | 1            |
| keda.maxReplicas                                                                                                                                                  | Maximum amount of Replicas                                                                                                                         | 10           |
| keda.pollingInterval                                                                                                                                              | Interval of polling the metrics in seconds                                                                                                         | 30           |
| keda.authentication                                                                                                                                               | Controls the authentication between KEDA and the service it is autoscaling                                                                         | []           |
| keda.triggers                                                                                                                                                     | List of event sources that trigger the autoscaling                                                                                                 | []           |
...
| workers[].keda                                                                                                                                                    | Per-worker KEDA block. Same keys and rules as keda above, including fallback                                                                       | {}           |
...
Use `hooks[].deletePolicy` for named hooks that must rerun on every Argo CD sync. For migration hooks, set `before-hook-creation` so Argo CD recreates the Job before each sync instead of leaving the previous named Job in place.
